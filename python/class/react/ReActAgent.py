...
from react.Template import REACT_PROMPT_TEMPLATE
    ...
    def run(self, question: str):
        ...
        while current_step < self.max_step:
            ...
            if action.startswith("Finish"):
                ...
                match_finish = re.match(r"Finish\[(.*)\]", action, re.DOTALL)
                if match_finish:
                    final_answer = match_finish.group(1)
                    print(f"🎉 最终答案: {final_answer}")
                    return final_answer
                else:
                    print("❌ Finish指令解析失败，原始action内容：")
                    print(repr(action))
                    break
